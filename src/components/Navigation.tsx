            ...
            {tabs.map(({ id, label, icon: Icon }) => (
              <Link
                ...
                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 overflow-hidden group ${
                  activeTab === id
                    ? "bg-gradient-to-r from-orange-500/10 to-red-500/10 text-gray border border-border/50 border-red-500/50 sshadow-md"
                    : "text-muted-foreground hover:text-foreground "
                }`}
              >
                {activeTab !== id && (
                  <div className="absolute inset-0 text-gray bg-gradient-to-r from-orange-700/10 to-red-300/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                )}
                <Icon
